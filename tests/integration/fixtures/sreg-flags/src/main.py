...
#
# Uses non-inline helper functions so both ADD/SUB operands are runtime values
# from the callee's perspective, preventing the compiler from constant-folding
# the arithmetic and ensuring real ADD/SUB machine instructions are emitted.
#
# Key property: after each helper returns, only MOV and OUT/STS instructions
# run before BREAK. Neither modifies SREG, so the flag state at each BREAK
# directly reflects the last ADD or SUB executed inside the helper.
#
# Checkpoints:
#   1 -- add_u8(255, 1)  = 0   : ADD sets C=1, Z=1; global interrupts disabled
