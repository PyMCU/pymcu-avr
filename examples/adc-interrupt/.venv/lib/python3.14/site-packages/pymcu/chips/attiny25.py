...
# -----------------------------------------------------------------------------
#
# ATtiny25 -- 8-pin AVR, PORTB only (PB0-PB5 usable, PB5=RESET)
# 128B SRAM, 2KB flash, 128B EEPROM
# Note: ATtiny25 does NOT have a hardware multiplier (AVRe core).
...
device_info(chip="attiny25", arch="avr", ram_size=RAM_SIZE)

# ==========================================
#  Register Definitions (ATtiny25)
# ==========================================
# Register map is identical to ATtiny85; only SRAM size differs.
...
# Timer/Counter 1 (8-bit, ATtiny25 specific)
TCCR1:  ptr[uint8] = ptr(0x50)   # I/O 0x30
TCNT1:  ptr[uint8] = ptr(0x51)   # I/O 0x31
OCR1A:  ptr[uint8] = ptr(0x4E)   # I/O 0x2E
OCR1C:  ptr[uint8] = ptr(0x4F)   # I/O 0x2F
GTCCR:  ptr[uint8] = ptr(0x43)   # I/O 0x23
DT1A:   ptr[uint8] = ptr(0x45)   # I/O 0x25
DT1B:   ptr[uint8] = ptr(0x44)   # I/O 0x24
