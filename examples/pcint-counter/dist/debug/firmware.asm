; Generated by pymcuc for atmega328p
.equ RAMSTART = 0x0100
.equ _stack_base = RAMSTART
.equ inline1.write.data = _stack_base + 0
.equ inline2.uart_write.data = _stack_base + 1
.equ tmp_4 = _stack_base + 3
.equ tmp_6 = _stack_base + 4
.equ tmp_9 = _stack_base + 5

.org 0x0000
	RJMP	main
.org 0x0001
	RETI
.org 0x0002
	RETI
.org 0x0003
	RJMP	pcint0_isr
.org 0x0004
	RETI
.org 0x0005
	RETI
.org 0x0006
	RETI
.org 0x0007
	RETI
.org 0x0008
	RETI
.org 0x0009
	RETI
.org 0x000A
	RETI
.org 0x000B
	RETI
.org 0x000C
	RETI
.org 0x000D
	RETI
.org 0x000E
	RETI
.org 0x000F
	RETI
.org 0x0010
	RETI
.org 0x0011
	RETI
.org 0x0012
	RETI
.org 0x0013
	RETI
.org 0x0014
	RETI
.org 0x0015
	RETI
.org 0x0016
	RETI
.org 0x0017
	RETI
.org 0x0018
	RETI
.org 0x0019
	RETI

pcint0_isr:
; ISR prologue — save context
	PUSH	R16
	PUSH	R17
	PUSH	R18
	IN	R16, 0x3F
	PUSH	R16
; main.py:25:     GPIOR0[0] = 1
	SBI	0x1E, 0
; ISR epilogue — restore context
	POP	R16
	OUT	0x3F, R16
	POP	R18
	POP	R17
	POP	R16
	RETI
main:
	LDI	R16, high(0x08FF)
	OUT	0x3E, R16
	LDI	R16, low(0x08FF)
	OUT	0x3D, R16
	LDI	R28, low(_stack_base)
	LDI	R29, high(_stack_base)
; main.py:29:     btn  = Pin("PB0", Pin.IN, pull=Pin.PULL_UP)
; main.py:22: 
; main.py:6: # Hardware: Arduino Uno
; main.py:7: #   - Button on PB0 (Arduino pin 8), active-low with internal pull-up
; main.py:10: # PCINT0 fires on any edge of any PB pin enabled in PCMSK0.
	CBI	0x04, 0
	SBI	0x05, 0
; main.py:30:     uart = UART(9600)
; main.py:39: 
	SBI	0x0A, 1
; main.py:40:     uart.println("PCINT COUNTER")
	CBI	0x0A, 0
; main.py:43: 
; main.py:44:     while True:
	LDI	R24, 103
	STS	0x00C4, R24
; main.py:45:         if GPIOR0[0] == 1:
	CLR	R24
	STS	0x00C5, R24
	LDI	R24, 6
	STS	0x00C2, R24
	LDI	R24, 24
	STS	0x00C1, R24
; main.py:33:     PCMSK0.value = 0x01
	LDI	R24, 1
	STS	0x006B, R24
; main.py:35:     PCICR[0] = 1
	LDS	R24, 0x0068
	ORI	R24, 1
	STS	0x0068, R24
; main.py:37:     GPIOR0[0] = 0
	CBI	0x1E, 0
; main.py:38:     asm("SEI")
SEI
; main.py:40:     uart.println("PCINT COUNTER")
	LDI	R30, low(__str_0 * 2)
	LDI	R31, high(__str_0 * 2)
	RCALL	__uart_send_z
L_65:
	LDS	R24, 0x00C0
	ANDI	R24, 32
	BREQ	L_BR_SKIP_0
	RJMP	L_66
L_BR_SKIP_0:
	RJMP	L_65
L_66:
	LDI	R24, 10
	STS	0x00C6, R24
	CLR	R24
	MOV	R4, R24
; main.py:44:     while True:
L_67:
; main.py:45:         if GPIOR0[0] == 1:
	SBIS	0x1E, 0
	RJMP	L_69
; main.py:46:             GPIOR0[0] = 0
	CBI	0x1E, 0
; main.py:48:             if btn.value() == 0:
	SBIS	0x03, 0
	RJMP	L_BIT_FALSE_1
	LDI	R24, 1
	RJMP	L_BIT_DONE_2
L_BIT_FALSE_1:
	CLR	R24
L_BIT_DONE_2:
	MOV	R16, R24
	CPI	R24, 0
	BREQ	L_BR_SKIP_3
	RJMP	L_70
L_BR_SKIP_3:
; main.py:49:                 count = count + 1
	INC	R4
; main.py:50:                 uart.write_str("COUNT:")
	LDI	R30, low(__str_1 * 2)
	LDI	R31, high(__str_1 * 2)
	RCALL	__uart_send_z
; main.py:51:                 uart.write((count / 10) + 48)
	MOV	R24, R4
	LDI	R18, 10
	RCALL	__div8
	MOV	R16, R24
	SUBI	R24, 208
	STD	Y+1, R24
L_102:
	LDS	R24, 0x00C0
	ANDI	R24, 32
	BREQ	L_BR_SKIP_4
	RJMP	L_103
L_BR_SKIP_4:
	RJMP	L_102
L_103:
	LDD	R24, Y+1
	STS	0x00C6, R24
; main.py:52:                 uart.write((count % 10) + 48)
	MOV	R24, R4
	LDI	R18, 10
	RCALL	__mod8
	MOV	R16, R24
	SUBI	R24, 208
	STD	Y+1, R24
L_106:
	LDS	R24, 0x00C0
	ANDI	R24, 32
	BREQ	L_BR_SKIP_5
	RJMP	L_107
L_BR_SKIP_5:
	RJMP	L_106
L_107:
	LDD	R24, Y+1
	STS	0x00C6, R24
; main.py:53:                 uart.write('\n')
L_110:
	LDS	R24, 0x00C0
	ANDI	R24, 32
	BREQ	L_BR_SKIP_6
	RJMP	L_111
L_BR_SKIP_6:
	RJMP	L_110
L_111:
	LDI	R24, 10
	STS	0x00C6, R24
L_70:
L_69:
	RJMP	L_67

; --- Flash String Pool (LPM+Z UART send) ---
__uart_send_z:
__usendz_loop:
	LPM	R24, Z+
	TST	R24
	BREQ	__usendz_done
__usendz_wait:
	LDS	R25, 0x00C0
	SBRS	R25, 5
	RJMP	__usendz_wait
	STS	0x00C6, R24
	RJMP	__usendz_loop
__usendz_done:
	RET

__str_1:
.db 67, 79, 85, 78, 84, 58, 0
__str_0:
.db 80, 67, 73, 78, 84, 32, 67, 79, 85, 78, 84, 69, 82, 0

; --- PyMCU AVR Math Runtime ---
; -----------------------------------------------------------------------------
; PyMCU AVR Math Runtime
; -----------------------------------------------------------------------------
;
; __div8: Unsigned 8-bit Division
;   Input:  R24 (Dividend), R18 (Divisor)
;   Output: R24 (Quotient), R25 (Remainder)
;   Clobbers: R25
;
; __mod8: Unsigned 8-bit Modulo
;   Input:  R24 (Dividend), R18 (Divisor)
;   Output: R24 (Remainder)
;   Clobbers: R25
;
; -----------------------------------------------------------------------------

__div8:
    push r16
    ldi r16, 8      ; 8 iterations: one per dividend bit
    clr r25         ; Remainder = 0
    clc             ; Clear carry

__div8_loop:
    rol r24         ; Shift dividend left, MSB into Carry
    rol r25         ; Shift remainder left, Carry into LSB
    brcs __div8_sub ; Remainder overflowed (>= 256) -> must subtract
    cp r25, r18     ; Compare Remainder with Divisor
    brcs __div8_next; Remainder < Divisor: quotient bit = 0

__div8_sub:
    sub r25, r18    ; Remainder -= Divisor
    sec             ; Quotient bit = 1
    rjmp __div8_dec

__div8_next:
    clc             ; Quotient bit = 0

__div8_dec:
    dec r16
    brne __div8_loop

    rol r24         ; Final ROL shifts last quotient bit into R24

    pop r16
    ret

__mod8:
    rcall __div8
    mov r24, r25    ; Move remainder to result register
    ret

