; Generated by pymcuc for atmega328p
.equ RAMSTART = 0x0100
.equ _stack_base = RAMSTART
.equ inline3_uart_read_result = _stack_base + 0

.org 0x0000
main:
	LDI	R16, high(0x08FF)
	OUT	0x3E, R16
	LDI	R16, low(0x08FF)
	OUT	0x3D, R16
	LDI	R28, low(_stack_base)
	LDI	R29, high(_stack_base)
; main.py:29:     led = DigitalInOut(board.LED)
; main.py:39:         led.value = 0
; main.py:22: import board
; main.py:8: #
; main.py:18: # Wiring:
; main.py:28: def main():
	CLR	R24
	TST	R24
	BRNE	L_BR_SKIP_2
	RJMP	L_BIT_WRITE_SKIP_0
L_BR_SKIP_2:
	SBI	0x04, 5
	RJMP	L_BIT_WRITE_DONE_1
L_BIT_WRITE_SKIP_0:
	CBI	0x04, 5
L_BIT_WRITE_DONE_1:
	MOV	R6, R24
	CLR	R24
	MOV	R5, R24
; main.py:30:     led.direction = Direction.OUTPUT
	LDI	R24, 1
	MOV	R5, R24
	TST	R24
	BRNE	L_BR_SKIP_5
	RJMP	L_BIT_WRITE_SKIP_3
L_BR_SKIP_5:
	SBI	0x04, 5
	RJMP	L_BIT_WRITE_DONE_4
L_BIT_WRITE_SKIP_3:
	CBI	0x04, 5
L_BIT_WRITE_DONE_4:
; main.py:32:     uart = busio.UART(board.TX, board.RX, baudrate=9600)
; main.py:23: import busio
; main.py:39:         led.value = 0
	SBI	0x0A, 1
	CBI	0x0A, 0
	LDI	R24, 103
	STS	0x00C4, R24
	CLR	R24
	STS	0x00C5, R24
	LDI	R24, 6
	STS	0x00C2, R24
	LDI	R24, 24
	STS	0x00C1, R24
	MOV	R7, R24
; main.py:33:     print("READY")
	LDI	R30, low(__str_0 * 2)
	LDI	R31, high(__str_0 * 2)
	RCALL	__uart_send_z
	LDI	R30, low(__str_1 * 2)
	LDI	R31, high(__str_1 * 2)
	RCALL	__uart_send_z
; main.py:35:     while True:
L_67:
L_72:
	LDS	R24, 0x00C0
	ANDI	R24, 128
	BREQ	L_BR_SKIP_6
	RJMP	L_73
L_BR_SKIP_6:
	RJMP	L_72
L_73:
	LDS	R24, 0x00C6
	MOV	R4, R24
; main.py:37:         led.value = 1
	SBI	0x05, 5
; main.py:38:         uart.write(byte)
; main.py:27: 
L_82:
	LDS	R24, 0x00C0
	ANDI	R24, 32
	BREQ	L_BR_SKIP_7
	RJMP	L_83
L_BR_SKIP_7:
	RJMP	L_82
L_83:
	MOV	R24, R4
	STS	0x00C6, R24
; main.py:39:         led.value = 0
	CBI	0x05, 5
	RJMP	L_67

; --- Flash String Pool (LPM+Z UART send) ---
__uart_send_z:
__usendz_loop:
	LPM	R24, Z+
	TST	R24
	BREQ	__usendz_done
__usendz_wait:
	LDS	R25, 0x00C0
	SBRS	R25, 5
	RJMP	__usendz_wait
	STS	0x00C6, R24
	RJMP	__usendz_loop
__usendz_done:
	RET

__str_1:
.db 10, 0
__str_0:
.db 82, 69, 65, 68, 89, 0
